<script lang="ts">
	...
	let tokenCollapsed = $state<Record<string, boolean>>({});
	...
	function isTokenCollapsed(token: string): boolean {
		return tokenCollapsed[tokenKey(token)] ?? true;
	}

	function toggleTokenCollapsed(token: string) {
		const key = tokenKey(token);
		tokenCollapsed[key] = !(tokenCollapsed[key] ?? true);
	}
...
>
	...
	<section>
		...
		<dl>
			...
			<div class="row">
				<dt data-fn="getAavePool() — Aave lending pool address where idle funds are deposited">
					Aave Pool
				</dt>
				<dd>
	...
	{#each data.tokens as t, i (t.token)}
		...
		<section class="token-section" class:token-collapsed={isTokenCollapsed(t.token)}>
			<div class="token-header">
				<button
					class="token-toggle"
					aria-expanded={!isTokenCollapsed(t.token)}
					aria-label={`${isTokenCollapsed(t.token) ? 'Expand' : 'Collapse'} ${sym || `Token ${i}`}`}
					title={isTokenCollapsed(t.token) ? 'Expand token' : 'Collapse token'}
					onclick={() => toggleTokenCollapsed(t.token)}
				>
					<span class="toggle-icon" aria-hidden="true">
						{isTokenCollapsed(t.token) ? '▸' : '▾'}
					</span>
					<span class="token-title">
						<span>{sym ? `${sym} —` : `Token ${i} —`}</span>
						<span class="token-address" title={t.token}>{shortenAddress(t.token)}</span>
					</span>
				</button>
				<div class="token-summary">
					<span class:badge-green={!t.isIdle} class:badge-gray={t.isIdle}>
						{t.isIdle ? 'Idle' : 'In Aave'}
					</span>
					<span class:badge-green={!t.assetData.forceIdle} class:badge-gray={t.assetData.forceIdle}>
						Force {t.assetData.forceIdle ? 'On' : 'Off'}
					</span>
			...
			{#if !isTokenCollapsed(t.token)}
				<dl>
					<div class="row">
						<dt
							data-fn="getAssetStatus() — true = token held in vault (not deposited to Aave); false = deposited to Aave earning yield"
						>
							Is Idle
						</dt>
					...
					<div class="row">
						<dt
							data-fn="getAssetData() → forceIdle — Whether this token is forced to stay idle in this vault"
						>
							Force Idle
						</dt>
						<dd>
									...
									editVault(
										...
										[
											{ key: 'token', label: 'Token', type: 'address', value: t.token },
											{
												...
												label: 'Force idle',
												type: 'bool',
												value: String(t.assetData.forceIdle)
											}
										],
					...
					<div class="row">
						<dt
							data-fn="getYieldAccumulator() — Normalized yield accumulator in RAY (10²⁷). Starts at 1.000000×, increases as Aave interest accrues"
						>
							Yield Accumulator
						</dt>
						<dd>
							{(Number(t.yieldAccumulator) / 1e27).toFixed(6)}<span class="unit">×</span>
							{#if pt}{@const d = bigDiff(t.yieldAccumulator, pt.yieldAccumulator)}{#if d}<span
										class="diff {d.dir}">{d.label}</span
									>{/if}{/if}
						</dd>
					</div>
					<div class="row">
						<dt data-fn="getAssetData() → totalBalance — Total token balance tracked by the vault">
							Total Balance
						</dt>
						<dd>
							{t.assetData.totalBalance.toString()}
							{#if pt}{@const d = bigDiff(
									t.assetData.totalBalance,
									pt.assetData.totalBalance
								)}{#if d}<span class="diff {d.dir}">{d.label}</span>{/if}{/if}
						</dd>
					</div>
					<div class="row">
						<dt data-fn="getAssetData() → totalShares — Total vault shares tracked for this token">
							Total Shares
						</dt>
						<dd>
							{t.assetData.totalShares.toString()}
							{#if pt}{@const d = bigDiff(
									t.assetData.totalShares,
									pt.assetData.totalShares
								)}{#if d}<span class="diff {d.dir}">{d.label}</span>{/if}{/if}
						</dd>
					</div>
				</dl>
				...
				<div class="sub-section">
					...
					<dl>
						<div class="row">
							<dt
								data-fn="getAaveTokenSupport() → supported — Whether Aave supports this token as a deposit asset"
							>
								Supported
							</dt>
						...
						<div class="row">
							<dt
								data-fn="getAaveTokenSupport() → frozen — Whether Aave marks this asset as frozen"
							>
								Frozen
							</dt>
						...
						<div class="row">
							<dt
								data-fn="getAaveTokenSupport() → paused — Whether Aave marks this asset as paused"
							>
								Paused
							</dt>
